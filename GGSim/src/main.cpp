...
int main()
{
    ...
    {
        auto obj = world.entityManager.spawn<TransformComponent, ShapeComponent, PhysicsComponent>();
        obj.getComponent<ShapeComponent>()->setShape(Box({ 50, 1, 50 }));
        obj.getComponent<TransformComponent>()->pos += Vec3_t(0, i, 0);
        // obj.getComponent<PhysicsComponent>()->vel = { 5, 0, 0 };

        // obj.getComponent<PhysicsComponent>()->angularVel = { 0, 0, 1 };
        // obj.getComponent<TransformComponent>()->rotateLocal(M_PI_4 / 2.f, { 0, 0, 3 });
        // obj.getComponent<TransformComponent>()->rotateLocal(M_PI_4, { 0, 1, 0 });
    }
    ...
    while (winM.isOpen())
    {
        ...
        if (glfwGetMouseButton(winM.win, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS && !spawn)
        {
            spawn    = true;
            auto box = world.entityManager.spawn<TransformComponent, ShapeComponent, PhysicsComponent>();
            box.getComponent<ShapeComponent>()->setShape(Box({ 1, 3, 1 }));
            box.getComponent<PhysicsComponent>()->setMass(50);
            box.getComponent<PhysicsComponent>()->vel   = winM.orient * Vec3_t(0, 0, -1) * 5.f;
