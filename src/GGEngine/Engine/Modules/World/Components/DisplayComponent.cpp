...
C2_TYPE DisplayShape::getType() const { return type; }
...
CircleDisplayShape::CircleDisplayShape() : DisplayShape(C2_TYPE::C2_CIRCLE)
{
    ...
    for (uint8_t i = 0; i <= vertices; ++i)
        shape.append(sf::Vector2f(std::cos(step * i), std::sin(step * i)));
}

void CircleDisplayShape::draw(sf::RenderTarget& target,
...
bool CircleDisplayShape::copyConfiguration(const ShapeBase& shapeToCpy)
{
    if (shapeToCpy.getType() == C2_TYPE::C2_CIRCLE) {
        const CircleShape& castedShapeToCpy =
            static_cast<const CircleShape&>(shapeToCpy);
...
void CircleDisplayShape::setRadius(float radius) { this->radius = radius; }
...
void PolygonDisplayShape::draw(sf::RenderTarget& target,
                               sf::RenderStates states) const
{
    target.draw(shape, states);
}
...
bool PolygonDisplayShape::copyConfiguration(const ShapeBase& shapeToCpy)
{
    if (shapeToCpy.getType() == C2_TYPE::C2_POLY) {
        const PolygonShape& castedShapeToCpy =
            static_cast<const PolygonShape&>(shapeToCpy);
        setupVertices(castedShapeToCpy.getDetails().verts,
                      castedShapeToCpy.getDetails().count);
...
void DisplayComponent::takeShape(const ShapeBase& shape)
{
    if (displayShape->getType() != shape.getType()) {
        ...
        if (newShape)
            ...
            EPP_ASSERT_M(newShape, "Used unsupported shape type");
    }
...
DisplayShape& DisplayComponent::getShape() { return *displayShape; }

const DisplayShape& DisplayComponent::getShape() const { return *displayShape; }
